...
import { MINT_SIZE, createInitializeMintInstruction, getMinimumBalanceForRentExemptMint, getAssociatedTokenAddress, createAssociatedTokenAccountInstruction, createMintToInstruction } from '@solana/spl-token';
...
import { TOKEN_PROGRAM_ID, getMint, getAccount, getOrCreateAssociatedTokenAccount, mintTo, 
    transfer, Account,createMint, TokenInstruction   } from '@solana/spl-token';
...
const MyWallet: React.FC = () => {
    ...
    window.Buffer = window.Buffer || require("buffer").Buffer;
        ...
        const toWallet = wallet?.publicKey;
        const connections = new Connection(clusterApiUrl('testnet'), 'confirmed');

        let fromTokenAccount: Account; 
        let mint: PublicKey;
    
        async function createToken() {
            const fromAirdropSignature = await connections.requestAirdrop(fromWallet.publicKey, LAMPORTS_PER_SOL);
            await connections.confirmTransaction(fromAirdropSignature);
            ...
            const mintInfo = await getMint(connections, mint);
            console.log(mintInfo.supply);
            
            // get the amount of tokens left in the account
            const tokenAccountInfo = await getAccount(connections, fromTokenAccount.address);
            console.log(tokenAccountInfo.amount);
        
    
      
            // Get the token account of the toWallet address, and if it does not exist, create it
            const toTokenAccount = await getOrCreateAssociatedTokenAccount(connections, fromWallet, mint, toWallet);
            ...
            console.log(`finished transfer with ${signatures}`);
                            ...
                            >
                                ...
                                >
                                    <div style={{ margin: '10px 0' }}>
                                        ...
                                        <button style={styles.btnStyle} onClick={createToken}>
                                            create token
                                        </button>
                                    </div>
                                    <div
